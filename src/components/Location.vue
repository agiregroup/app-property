<script setup lang="ts">
 ...
 const store = usePropertiesStore()
  ...
  onMounted(async () => {
    await store.charger()
    biensFiltres.value = store.toutes.filter((bien: Property) => bien.type?.toLowerCase() === 'location')
...
<template>
  <div class="flex flex-col gap-4 px-4">
    ...
    <TablesBien :biens="biensAffiches" titre="Biens en location"/>
    
    
        <!--<div class="text-blue-600 text-sm">
      {{ biensFiltres.length }} biens filtrés
    </div>-->


    <Pagination
        :page="pagination.pageActuelle"
        :totalPages="pagination.nombreDePages"
        @changer-page="pagination.changerPage"
      />
